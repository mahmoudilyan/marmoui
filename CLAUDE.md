# Marmo UI — Claude Code Project Memory

Auto-loaded by Claude Code for every session in this repo. Stack, layout, and the required workflow for generating UI that matches the design system.

> Non-Claude agents: [AGENTS.md](AGENTS.md) carries the same conventions.
...
| Claude Code plugin + marketplace | [tools/claude-plugin/](tools/claude-plugin/) · [.claude-plugin/marketplace.json](.claude-plugin/marketplace.json) |
...
The **Marmo UI MCP** (`https://mcp.marmoui.com/mcp` — connect via `npx marmoui init` or the plugin) is the single source of truth for component APIs, props, patterns, and generation rules.
...
When writing or editing UI built with `@marmoui/ui` (the `marmo-ui` skill auto-triggers this if the plugin is installed):

1. **Verify freshness.** `mcp__marmo-ui__get_knowledge_version` once per session.
2. **Read the rules.** `mcp__marmo-ui__get_design_guidelines` once per session.
3. **Find the right primitive.** Known multi-component layout → `mcp__marmo-ui__get_pattern`; otherwise `mcp__marmo-ui__search_components` + `mcp__marmo-ui__get_component_info`.
4. **Write the code.** Imports only from `@marmoui/ui` and the chosen icon library.
5. **Validate.** `mcp__marmo-ui__review_generated_code` on the final output; fix every error and warning before reporting done.
