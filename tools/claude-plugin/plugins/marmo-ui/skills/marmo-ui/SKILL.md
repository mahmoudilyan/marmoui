...
- **Imports.** `@marmoui/ui` for components; the user's chosen icon library for icons (default `@phosphor-icons/react`; also `react-icons/md`, `lucide-react`, `@tabler/icons-react`, `@heroicons/react`, `react-icons/fi`). Never `@chakra-ui/react`, `@radix-ui/*` directly, `@mui/*`, `@/components/ui/*`, and never mix icon libraries.
