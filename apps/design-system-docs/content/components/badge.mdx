...
Use `variant="success"` / `"warning"` / `"destructive"` to show row status in a DataTable cell renderer.

```tsx
// Status badge in a TanStack Table cell
{
  accessorKey: 'status',
  header: 'Status',
  cell: ({ row }) => {
    const map = {
      ...
      inactive: { variant: 'normal'      as const, label: 'Inactive' },
    };
    const { variant, label } = map[row.original.status];
...
<Badge variant="success" leftIcon={<MdCheck className="h-3 w-3" />}>Verified</Badge>
<Badge variant="warning" leftIcon={<MdWarning className="h-3 w-3" />}>Review needed</Badge>
```

## Common Mistakes

- Using `variant="danger"` — the correct value is `variant="destructive"`
- Using Badge as an interactive element — it has no click handler; use Button instead
- Long text in badges — keep to 1–3 words maximum
