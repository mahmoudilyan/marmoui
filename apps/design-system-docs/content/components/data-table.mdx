...
- **`dataType` is singular** — pass `dataType="contact"`, not `"contacts"`. The table pluralizes it for the count text and empty state ("Showing 4 of 20 contacts", "No contacts found", "creating your first contact").
- **Do not build your own toolbar above DataTable.** Search, columns menu, filters, and the count text are built in. The page's primary action goes in `PageSection`'s `primaryAction`, not in a row above the table.
